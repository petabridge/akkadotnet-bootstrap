// -----------------------------------------------------------------------
// <copyright file="PcfEnvironment.cs" company="Petabridge, LLC">
...
namespace Akka.Bootstrap.PCF
{
    ...
    public sealed class PcfEnvironment
    {
        ...
        /// <summary>
        ///     The internal IP address of this instance.
        /// </summary>
        public IPAddress CF_INSTANCE_INTERNAL_IP { get; }

        /// <summary>
        ///     The external, or host-side, port corresponding to the internal, or container-side, port with value
        ///     <see cref="PORT" />.
        ...
        /// <summary>
        ///     The list of mappings between internal, or container-side, and external, or host-side, ports allocated to the
        ///     instance’s container.
        ///     Not all of the internal ports are necessarily available for the application to bind to, as some of them may be used
        ///     by
        ...
        {
            sb.AppendLine($"CF_INSTANCE_ADDR: {CF_INSTANCE_ADDR}")
                .AppendLine($"CF_INSTANCE_GUID: {CF_INSTANCE_GUID}")
                .AppendLine($"CF_INSTANCE_INDEX: {CF_INSTANCE_INDEX}")
                .AppendLine($"CF_INSTANCE_IP: {CF_INSTANCE_IP}")
                .AppendLine($"CF_INSTANCE_INTERNAL_IP: {CF_INSTANCE_INTERNAL_IP}")
                .AppendLine($"CF_INSTANCE_PORT: {CF_INSTANCE_PORT}")
                .AppendLine("CF_INSTANCE_PORTS: ");

            foreach (var portMapping in CF_INSTANCE_PORTS)
                sb.AppendLine("  " + portMapping);
